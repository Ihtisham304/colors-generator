...
@api_view(['DELETE'])
@permission_classes([IsAuthenticated])
def delete_color_hash(request, id):
    # Fetch the ColorHash by ID and ensure it belongs to the authenticated user
    color_hash = get_object_or_404(ColorHash, id=id, user=request.user)
    
    # If it's a DELETE request, proceed with deletion
    if request.method == 'DELETE':
        color_hash.delete()
        return Response({"message": "ColorHash deleted successfully"}, status=status.HTTP_204_NO_CONTENT)
